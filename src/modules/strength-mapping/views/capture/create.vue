              ...
              >
                <ul class="my-2">
                  <div v-for="cluster in clusters" :key="cluster.name">
                    <li v-for="typology in cluster.groups" :key="typology">
                      <button
                        type="button"
...
const onChooseIkigai = (cluster: any, ikigai: string) => {
  if (!cluster.ikigai.includes(ikigai)) {
    cluster.ikigai.push(ikigai)
  } else {
    cluster.ikigai = cluster.ikigai.filter((e: any) => e !== ikigai)
  }
}

const isIkigaiChoosen = (cluster: any, ikigai: string) => {
  if (cluster.ikigai.includes(ikigai)) {
    return true
  }
  return false
